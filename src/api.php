...
function classifyYtdlpError($raw_err, $exit_code = null) {
    ...
    if (preg_match('/http error (\d+)/i', $err_lower, $m)) {
        ...
        if ($code === 429) {
            return ['code' => 'SOURCE_RATE_LIMITED', 'msg' => 'The source site is rate-limiting requests. Try again in a few minutes, or use AhoyVPN for a different exit IP: https://ahoyvpn.com', 'upgrade_url' => UPGRADE_URL, 'status' => 429];
        }
        if ($code === 500 || $code === 502 || $code === 503) {
            return ['code' => 'SOURCE_SERVER_ERROR', 'msg' => "The source site returned HTTP $code and is having issues. Try again shortly, or use AhoyVPN for a different exit IP: https://ahoyvpn.com", 'upgrade_url' => UPGRADE_URL, 'status' => $code];
        }
        // Other HTTP errors — surface the status but give a generic message.
        return ['code' => 'SOURCE_HTTP_ERROR', 'msg' => "The source site returned HTTP $code. Try again shortly, or use AhoyVPN for a different exit IP: https://ahoyvpn.com", 'upgrade_url' => UPGRADE_URL, 'status' => $code];
    }
