...
define('YTDLP_PATH', getenv('YTDLP_PATH') !== false && getenv('YTDLP_PATH') !== '' ? getenv('YTDLP_PATH') : '/usr/local/bin/yt-dlp');
...
// that do not support the modern Reporting API; that server-level directive also
// covers older Chromium (84-93) that support report-to but not Reporting-Endpoints.
// The report-to header below provides endpoint-group configuration for older clients.
...
if ($is_rate_limited) {
    ...
    if (time() - $data['t'] < $rate_window) {
        ...
        if ($data['c'] + 1 > $rate_limit) {
            ...
            'x_ffprobe_status' => 'skipped',
            // x_info_timeout / x_download_timeout: mirror the HTTP headers set above
            // (lines 638-641). Adding them to the body completes the "always present"
            // invariant documented in the README: every API response body includes
            // x_info_timeout and x_download_timeout. Consistent with the check, health,
...
function classifyYtdlpError($raw_err, $exit_code = null) {
    ...
    // for this case. "read at byte 0 timeout" is the same class — the TCP connection
    // established but the source sent no data within the socket timeout. The negative
    // lookahead (?!\\s) checks what comes immediately after "timeout" — rejecting
    // "connection timed out <suffix>" (e.g. "connection timed out after 45s") which is
...
$validation = function(string $action) use($request_id, $sendDailyLimitHeaders) {
    ...
    if (!isValidUrl($url)) {
        ...
        echo json_encode([
            ...
            // Adding them to the body completes the "always present" invariant documented
            // in the README: every API response body includes x_info_timeout and x_download_timeout.
            // Consistent with the check, health, SERVICE_UNAVAILABLE, MISSING_URL,
            // and info/download response bodies.
            'x_info_timeout' => INFO_TIMEOUT,
            'x_download_timeout' => DOWNLOAD_TIMEOUT,
        ], JSON_INVALID_UTF8_SUBSTITUTE);
...
if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
    ...
    echo json_encode([
        ...
        'platform' => null,
        // 'video_url' mirrors source_url — null because METHOD_NOT_ALLOWED fires
        // before any URL processing. Consistent with MISSING_URL and INVALID_URL.
        'video_url' => null,
        ...
        // x_download_timeout. Consistent with MISSING_URL, NOT_ACCEPTABLE, UNKNOWN_ACTION,
...
if (in_array($action, $json_actions, true) && $accept !== '' && $accept !== '*/*' && !preg_match('/application\/json/i', $accept)) {
    ...
    echo json_encode([
        ...
        // (it is an Accept-header validation failure, not a URL validation failure).
        'source_url' => null,
        // 'video_url' mirrors source_url — null because NOT_ACCEPTABLE fires
        // before any URL processing. Consistent with MISSING_URL and INVALID_URL.
        'video_url' => null,
    ...
    case 'info': {
        ...
        if (!$unlimited) {
            ...
            if (!flock($daily_fp, LOCK_EX)) {
                ...
                header('Cache-Control: no-store');
                http_response_code(503);
                // X-DailyLimit-*: -1 sentinels since the quota file could not be
                // locked. Mirrors the fopen failure block above for header parity.
                header('X-DailyLimit-Limit: -1');
                header('X-DailyLimit-Remaining: -1');
                header('X-DailyLimit-Reset: -1');
                header('X-DailyLimit-Window: unavailable');
        ...
        if (!$parsed) {
            ...
            header('X-Info-Timeout: ' . INFO_TIMEOUT);
            header('X-Download-Timeout: ' . DOWNLOAD_TIMEOUT);
            // X-FFProbe-Status: skipped — ffprobe never runs in the parse_formats
            // failure path (yt-dlp ran but parseFormats returned null).
            header('X-FFProbe-Status: skipped');
            header('X-FFProbe-Timeout: ' . FFPROBE_TIMEOUT);
            header('Reporting-Endpoints: csp-report="/csp-report"');
            header('Report-To: {"group":"csp-report","max_age":86400,"endpoints":[{"url":"/csp-report"}]}');
        ...
        if (isset($parsed['error'])) {
            ...
            header('X-Info-Timeout: ' . INFO_TIMEOUT);
            header('X-Download-Timeout: ' . DOWNLOAD_TIMEOUT);
            // CSP triple + Reporting-Endpoints — consistent with every other API error
...
// ─── Sanitize derived filename ───
        ...
        if ($filesize === false || $filesize === 0 || !$actual_file || !is_file($actual_file)) {
            ...
            header('X-Download-Timeout: ' . DOWNLOAD_TIMEOUT);
            header('X-Info-Timeout: ' . INFO_TIMEOUT);
            // X-FFProbe-Status: ffprobe was never reached — yt-dlp exited 0 (success code)
            // but produced no file or an empty/zero-byte file, so there was nothing for
    ...
    case 'client-error': {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            ...
            echo json_encode([
                ...
                'quota_reset_unix' => -1,
                // server_time: ISO 8601 + Unix for client clock synchronization.
                // Present on all other API responses (MISSING_URL, NOT_ACCEPTABLE,
    ...
    case 'analytics': {
        ...
        if ($raw_body === false || $raw_body === '') {
            ...
            // Use 'D, d M Y H:i:s GMT' format (RFC 7231) — consistent with all other
            // API responses including the health, check, client-error, and info actions.
            header('X-Server-Time: ' . gmdate('D, d M Y H:i:s') . ' GMT');
            header('X-Server-Time-Unix: ' . time());
            break;
        }
    ...
    default: {
        ...
        echo json_encode([
            ...
            // quota_limit: set to the configured daily limit (always known, even pre-gate)
            // to match MISSING_URL which also sets quota_limit even though quota_remaining
            // is -1. API consumers can always determine the configured limit from the body.
            'quota_remaining' => -1,
            'quota_limit' => getDailyQuotaLimit(),
            'quota_reset' => (new DateTime('tomorrow midnight', new DateTimeZone('UTC')))->format('c'),
            'quota_reset_unix' => (new DateTime('tomorrow midnight', new DateTimeZone('UTC')))->getTimestamp(),
