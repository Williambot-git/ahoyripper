...
| `QUOTA_DIR` | `/tmp/quota` | Directory for daily-quota and rate-limit state files (`ahoyrip_daily_*`, `ahoyrip_rate_*`, `ahoyrip_dl_rate_*`). Docker deployments use a tmpfs mount at `/tmp/quota` so quota state is stored in memory and wiped on container restart (preventing quota bypass via restart). Non-Docker deployments default to `/tmp`. Mount a persistent volume (e.g. `/var/run/ahoyripper/quota`) for quota to survive restarts. |
