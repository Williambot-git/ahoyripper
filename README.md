...
| `FFPROBE_PATH` | `/usr/bin/ffprobe` | Path to the ffprobe binary used for post-download codec/resolution verification. Override to use a custom ffprobe path (e.g. `/usr/local/bin/ffprobe` on macOS or custom Docker images). The version cache is keyed on this path, so changing it also invalidates the ffmpeg version cache. |
...
| `playlist` | `0` | Set to `1` to fetch info for all videos in a playlist (`--yes-playlist`). By default (`playlist=0`), `--no-playlist` is passed to yt-dlp so single-video URLs always return one result regardless of whether the URL is part of a playlist. Each video in a playlist counts as a separate rip. |
