...
| `YTDLP_PATH` | `/usr/local/bin/yt-dlp` | Path to the yt-dlp binary. Override when yt-dlp is in a non-standard location (e.g. `/usr/bin/yt-dlp` on some systems). Changing this invalidates the yt-dlp version cache. Docker users: set in docker-compose `environment:` or `.env`. |
