...
| `INFO_TIMEOUT` | `45` | PHP constant name for the same timeout as `YTDLP_TIMEOUT` — exposed as the `X-Info-Timeout` response header on every API response. Both `YTDLP_TIMEOUT` and `INFO_TIMEOUT` refer to the same value; `YTDLP_TIMEOUT` is the env var, `INFO_TIMEOUT` is the PHP constant name referenced in client-facing headers. |
