...
| `RATE_LIMIT` | `30` | Request rate limit per IP per minute. Applied to the `info` action only — the `download` action has its own separate limit governed by `DL_RATE_LIMIT` (default 10/min). nginx enforces a separate 30r/m gate before requests reach PHP; this PHP-layer limit is the per-action ceiling for info. Tune independently of the nginx limit to discipline clients that pass through nginx but exceed per-action quotas. |
