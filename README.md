...
| `INFO_TIMEOUT` | `45` | PHP constant name for the same timeout as `YTDLP_TIMEOUT` — exposed as the `X-Info-Timeout` response header on every API response. Both `YTDLP_TIMEOUT` and `INFO_TIMEOUT` refer to the same value; `YTDLP_TIMEOUT` is the env var name, `INFO_TIMEOUT` is the PHP constant used in code and client-facing headers. |
...
| `FFMPEG_PATH` | `/usr/bin/ffmpeg` | Path to the ffmpeg binary used by yt-dlp for post-processing (merging streams, transcoding, etc.). Override when ffmpeg is in a non-standard location (e.g. `/usr/local/bin/ffmpeg`). Passed to yt-dlp as `--ffmpeg-location` to ensure yt-dlp can locate it even when it is not in the system PATH. |
...
| `QUOTA_DIR` | `/tmp` | Directory for daily-quota and rate-limit state files (`ahoyrip_daily_*`, `ahoyrip_rate_*`, `ahoyrip_dl_rate_*`). Defaults to `/tmp` so Docker container restarts wipe quota state — preventing quota bypass via container restart. Mount a persistent volume (e.g. `/var/run/ahoyripper/quota`) for quota to survive restarts. |
...
| `API_KEY_LEN` | `64` | Maximum API key length in characters. Keys exceeding this length are rejected with HTTP 401 before the timing-safe comparison. This prevents pathological inputs from allocating long strings unnecessarily. Override via `API_KEY_LEN` env var if using a longer key (e.g. `128`). |
...
| `X-Download-Timeout` | Server-side download timeout in seconds (integer). Analogous to `X-Info-Timeout` but for download operations; present on all API responses for consistent client retry logic regardless of action type. The value matches `DOWNLOAD_TIMEOUT` (default: 300 seconds). |
| `X-FFProbe-Status` | Post-download verification status. Always present on every API response (including `info`) for consistent header coverage. On `info` responses this is always `skipped` since ffprobe only runs after a download. On `download` responses the value is `success` (ffprobe confirmed a video stream), `skipped` (no video stream found, e.g. audio-only format), or `failed` (file corrupt/unreadable — quota is refunded). |
| `X-FFProbe-Timeout` | Server-side ffprobe verification timeout in seconds (integer). The maximum time ffprobe is allowed to run before the verification is aborted and treated as a timeout failure. Present on every API response for consistent client retry logic — on probe/read-only actions (check, health, analytics, default) this is always `FFPROBE_TIMEOUT` since ffprobe only runs after a download; on download responses it reflects the actual ffprobe timeout (default: 10 seconds, configurable via `FFPROBE_TIMEOUT` env var). Use this to distinguish `VERIFICATION_TIMEOUT` (ffprobe hung) from `VERIFICATION_FAILED` (ffprobe ran but found the file corrupt). |
...
| `X-FFProbe-Status` | Always `skipped` on check and health responses since ffprobe only runs after a download completes. Present on every API response so clients can always find this header. |
...
| `X-FFProbe-Status` | Post-download verification result: `success` (ffprobe confirmed a video stream in the downloaded file), `skipped` (ffprobe exited successfully but found no video stream — e.g. audio-only format, or the container was empty/malformed), or `failed` (ffprobe could not verify the file — corrupt, unreadable, or ffprobe execution error; quota is refunded). Always present on every download response. |
...
| `X-Requested-Format` | Echoes back the format ID the client sent in the `X-Requested-Format` request header (e.g. `bestvideo[height=1080]`, `22`, `bestaudio`). Present on every successful download response when the client sends the header. Absent when the header is not provided. Use this to correlate the client's original selection with the actual delivered file — compare against `X-Format-Substituted` to determine whether yt-dlp honoured the requested format. |
...
| `X-RateLimit-Window` | Window size in seconds for the shared request-rate limit (`60`). |
| `X-DL-RateLimit-Window` | Window size in seconds for the download-specific rate limit (`60`). |
...
| `INFO_TIMEOUT` | `45` | **Not an env var** — a PHP constant derived from `YTDLP_TIMEOUT`. Exposed as the `X-Info-Timeout` response header on every API response so clients know the server-side timeout. Set via `YTDLP_TIMEOUT` env var (e.g. `YTDLP_TIMEOUT=60`). |
...
| `FFMPEG_PATH` | `/usr/bin/ffmpeg` | Path to the ffmpeg binary used by yt-dlp for post-processing (merging streams, transcoding, etc.). Override when ffmpeg is in a non-standard location (e.g. `/usr/local/bin/ffmpeg`). Passed to yt-dlp as `--ffmpeg-location` to ensure yt-dlp can locate it even when not in the system PATH. |
| `FFPROBE_TIMEOUT` | `10` | Timeout in seconds for ffprobe post-download verification. ffprobe should finish in well under 10s for any real file; increase this when running on slow storage or with very large files that need extra time for codec probing. |
...
| `PROBE_CACHE_TTL` | `300` | Cache TTL in seconds for the yt-dlp connectivity probe in the health endpoint. The probe result is cached to avoid hammering YouTube with repeated health checks. Override via `PROBE_CACHE_TTL` env var in `.env` or docker-compose (e.g. `PROBE_CACHE_TTL=600` for a 10-minute cache). Use `max(1, ...)` clamping so the value is always at least 1 second. |
| `VERSION_CACHE_TTL` | `3600` | Cache TTL in seconds for yt-dlp and ffmpeg version checks. Both the yt-dlp version (checked on every `info`/`download` request) and ffmpeg version (checked on every `download` request) are cached to avoid repeated subprocess calls. Override via `VERSION_CACHE_TTL` env var in `.env` or docker-compose (e.g. `VERSION_CACHE_TTL=7200` for a 2-hour cache). Use `max(1, ...)` clamping so the value is always at least 1 second. |
| `API_KEY_LEN` | `64` | Maximum API key length in characters. Keys exceeding this length are rejected with HTTP 401 before the timing-safe comparison. This prevents pathological inputs from allocating long strings unnecessarily and provides an early-exit path for oversized payloads. Override via `API_KEY_LEN` env var if using a longer key (e.g. `128`). |
