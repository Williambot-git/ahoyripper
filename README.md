...
| `PLAUSIBLE_HOST` | `plausible.io` | Hostname of the self-hosted Plausible analytics server. Set to a custom Plausible domain (e.g. `plausible.mycompany.com`) to route browser analytics events through AhoyRipper's own `/src/api.php?action=analytics` proxy. Set to `''` (empty string) to disable analytics entirely — the endpoint returns 204 silently and no requests are forwarded. |
...
| `RATE_LIMIT` | `30` | Request rate limit per IP per minute. Applied to both `info` and `download` actions (both share the same rate limiter). nginx enforces a separate 30r/m gate before requests reach PHP; this PHP-layer limit is the per-action ceiling. Tune independently of the nginx limit to discipline clients that pass through nginx but exceed per-action quotas. |
...
| `500` | `PROC_OPEN_FAILED` | yt-dlp binary could not be started. The server is misconfigured — yt-dlp may not be installed, the path may be incorrect, or permissions may be missing. |
...
| `FFPROBE_TIMEOUT` | `10` | Timeout in seconds for ffprobe post-download verification. ffprobe should finish in well under 10s for any real file; increase this when running on slow storage or with very large files that need extra time for codec probing. |
| `HEALTH_PROBE_TIMEOUT` | `15` | Timeout in seconds for the `action=health&probe=1` connectivity probe. The probe is a lightweight `--dump-json` fetch on a known-short video, so 15s is plenty. Increase if the probe times out on slow networks or under heavy load. |
