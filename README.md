...
| `X-FFProbe-Timeout` | Server-side ffprobe verification timeout in seconds (integer). The maximum time ffprobe is allowed to run before the verification is aborted and treated as a timeout failure. Present on `download` responses where ffprobe was attempted. The value matches `FFPROBE_TIMEOUT` (default: 10 seconds, configurable via `FFPROBE_TIMEOUT` env var). Use this to distinguish `VERIFICATION_TIMEOUT` (ffprobe hung) from `VERIFICATION_FAILED` (ffprobe ran but found the file corrupt). |
...
| `X-FFProbe-Timeout` | Server-side ffprobe verification timeout in seconds (integer). The maximum time ffprobe is allowed to run before the verification is aborted and treated as a timeout failure. Present on download responses where ffprobe was attempted. The value matches `FFPROBE_TIMEOUT` (default: 10 seconds, configurable via `FFPROBE_TIMEOUT` env var). |
