...
| `AHOY_IMPERSONATE` | `chrome` | yt-dlp 2024.09+ browser TLS fingerprint impersonation target. Passed as `--impersonate chrome` to yt-dlp, spoofs browser TLS/ALPN fingerprints and dramatically reduces 403/422 bot-detection errors on protected sites (YouTube, Twitter, etc.). Set to empty string `''` to disable impersonation if needed. Not configurable via `.env` in production — edit the `AHOY_IMPERSONATE` constant in `src/api.php` directly for targeted testing. |
