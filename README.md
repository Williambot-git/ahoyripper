...
| `playlist` | `0` | Set to `1` to fetch info for all videos in a playlist (`--playlist true`). By default (`playlist=0`), `--playlist false` is passed to yt-dlp so single-video URLs always return one result regardless of whether the URL is part of a playlist. Each video in a playlist counts as a separate rip. |
