...
echo ""
echo "==> Checking JS does not inline-override formatGrid layout properties (regression)..."
# The formatGrid element's layout should be driven entirely by CSS (.format-grid rule).
# JS used to set display/flexDirection/flexWrap inline which could conflict with CSS,
# and was previously setting formatGrid.style.gap = '0' which overrode the CSS gap value.
# Verify NONE of these layout properties are set inline on formatGrid.
if grep -q "formatGrid\.style\.\(display\|flexDirection\|flexWrap\|gap\)\s*=" public/index.php; then
    echo "  ✗ JS sets inline layout property on formatGrid — CSS should be sole source of truth"
    exit 1
fi
echo "  ✓ JS does not inline-override formatGrid layout properties"
...
echo ""
echo "==> Checking client-error success response includes quota fields (API surface consistency)..."
# The client-error action (POST fire-and-forget) returns a 200 JSON response.
# quota_remaining/limit/reset fields must be present on ALL API responses for consistent
# client quota tracking — the check, health, and analytics actions all include them.
...
for field in "'upgrade_url'" "'quota_remaining'" "'quota_limit'" "'quota_reset'" "'quota_reset_unix'"; do
    if echo "$CE_SUCCESS" | grep -q "$field"; then
        echo "  ✓ client-error success response includes $field"
    else
        echo "  ✗ client-error success response missing $field"
        exit 1
    fi
done

echo ""
echo "==> Checking analytics.js SRI hash is correct and has proper base64 padding..."
ANALYTICS_SCRIPT="public/js/analytics.js"
ANALYTICS_TAG=$(grep "analytics.js" public/index.php | grep "integrity=")
if [ -z "$ANALYTICS_TAG" ]; then
    echo "  ✗ analytics.js script tag not found in index.php"
    exit 1
fi
# Extract integrity value from the tag
SRI_VALUE=$(echo "$ANALYTICS_TAG" | sed -n 's/.*integrity="\([^"]*\)".*/\1/p')
if [ -z "$SRI_VALUE" ]; then
